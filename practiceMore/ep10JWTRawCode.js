...
const { validateCookie } = require("./middlewares/cookieValidation"); // Importing the cookie validation middleware
...
app.post("/login", async (req, res) => {
  try {
    ...
    //new way of offloading the password validation to user.js model
    ...
    if (!isPasswordValid) {
      ...
      //old code
      // const token = jwt.sign({ _id: userData._id }, config.tokenSecret,{expiresIn: '1d'});
      // Create a JWT token with the user's ID

      //calling the getJWT method from userSchema to get the token
      const token = userData.getJWT();

      //why we use userData.getJWT() because we have defined the getJWT method in user.js model so we can call that method on the userData instance to get the token and userData already have all the vlues like _id, firstName, lastName, emailId, etc. so we can use that to create the token and there we are using this in model file so basically that this is basically have this data only.
...
//profile api with cookie authentication old without middleware
...
//validateCookie is middleware first code will go to validateCookie and then it will go to the next middleware or route handler
app.get("/profile",validateCookie, async (req, res) => {
